...
var debug = require('debug')('signalk-parser-nmea0183/PBVE');
...
function convertAlphasToInts(alphas) {
  ...
  for (var i = 0; i < alphas.length; i++) {
    var indexValue = ALPHA.indexOf(alphas[i]);
...
module.exports = function (input) {
  ...
  var data = parts[0];
  var productCode = data.substr(0, 1);
  ...
  var convertedValue = convertAlphasToInts(data.substr(28, 4));
  var oilPressure = convertToOilPressure(convertedValue);

  var oilPressureValue = {
